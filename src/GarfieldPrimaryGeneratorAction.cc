...
/*
  ...
  double x0 = 0.5 * (Garfield::RndmUniform() - 1);
  double y0 = 0.5 * (Garfield::RndmUniform() - 1);
  const double t0 = 0.;
  ...
  fParticleGun->SetParticleMomentumDirection(G4ThreeVector(dx0, dy0, dz0));
*/
  fParticleGun->SetParticlePosition(G4ThreeVector(0., 12*m, 0.));
